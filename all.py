...
def remove_background(nth_person, v2_real, a_real, frames_folder, save_dir, classes_file, weights_path):
    # ------------------------- Cấu Hình -------------------------
    conf_threshold = 0.5
    tracking_class = 0  
    ...
    # Mô hình DeepFill
    deepfill_weights_path = "./models/states_pt_places2.pth"  # Đường dẫn tuyệt đối
...
def paste_object_with_acceleration(nth_person, tracking_data, v1_real, v2_real, a_real, inpainted_frames_folder, original_frames_folder, gendata_folder):
    # ------------------------- Cấu Hình -------------------------
    os.makedirs(gendata_folder, exist_ok=True)
    FPS = 24  # Số khung hình trên giây của video

    print(f"Vận tốc đi bộ: {v1_real:.2f} m/s")
    print(f"Vận tốc tối đa: {v2_real:.2f} m/s")
    print(f"Gia tốc: {a_real:.2f} m/s²")
    ...
    while frame_idx < len(tracking_data):
        ...
        # Kiểm tra tính hợp lệ của frame_index
        if frame_index >= len(inpainted_frames_list) or frame_index >= len(original_frames_list):
            print(f"frame_index {frame_index} vượt quá phạm vi danh sách khung hình. Bỏ qua khung hình này.")
            frame_idx += 1
            continue
        ...
        # Kiểm tra kích thước hộp trước khi dán
        if paste_width > 0 and paste_height > 0:
            ...
            print(f"Kích thước hộp không hợp lệ: paste_width={paste_width}, paste_height={paste_height}")
            frame_idx += 1
            continue
        ...
        if acceleration_phase == 'accelerating':
            ...
            if constant_velocity_frame_count >= 10:
...
# ------------------------- Luồng Chính -------------------------
def main():
    parser = argparse.ArgumentParser(description="Xử lý xóa nền và dán đối tượng với thay đổi vận tốc.")
    parser.add_argument('--input_folder', type=str, required=True, help='Đường dẫn tới thư mục chứa các khung hình gốc (ví dụ: ./data_ext/training/01).')
    args = parser.parse_args()

    input_folder = args.input_folder  # Thư mục chứa khung hình gốc

    # Lấy tên thư mục đầu vào (ví dụ: "01" từ "./data_ext/training/01")
    input_folder_name = os.path.basename(os.path.normpath(input_folder))

    # Định nghĩa các thư mục đầu ra dựa trên tên thư mục đầu vào
    background_save_dir = os.path.join("./data_ext/gendata/background", input_folder_name)
    acceleration_save_dir = os.path.join("./data_ext/gendata/acceleration", input_folder_name)
    sudden_velocity_change_save_dir = os.path.join("./data_ext/gendata/sudden_velocity_change", input_folder_name)

    # Đường dẫn tuyệt đối cho các tham số khác
    classes_file = "data_ext/classes.names"  # Đường dẫn tuyệt đối
    weights_path = "./weights/yolov9-e.pt"  # Đường dẫn tuyệt đối
    ...
    tracking_data, v1_real = remove_background(
        nth_person, v2_real, a_real, 
    ...
    paste_object_with_acceleration(
        nth_person, tracking_data, v1_real, v2_real, a_real, 
        inpainted_frames_folder=background_save_dir, 
        original_frames_folder=input_folder, 
        gendata_folder=acceleration_save_dir
    )

    # Phương pháp 2: Dán đối tượng với vận tốc thay đổi đột ngột
    paste_object_with_sudden_velocity_change(
        nth_person, tracking_data, v1_real, v2_real, 
        inpainted_frames_folder=background_save_dir, 
        original_frames_folder=input_folder, 
        gendata_folder=sudden_velocity_change_save_dir
    )
