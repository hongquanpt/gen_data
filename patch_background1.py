...
gendata_folder = "/home/thieugt95/Desktop/gendata/01_background"
    ...
    for track in tracks:
        if track.is_confirmed():
            ...
            if track_id == target_track_id:
                ...
                # Calculate new coordinates for the paste area
                paste_x1 = max(0, x1 - 3)
                paste_y1 = max(0, y1 - 3)
                paste_x2 = min(frame.shape[1], x2 + 3)
                paste_y2 = min(frame.shape[0], y2 + 3)

                # Resize the extracted region to match the paste area
                region_width = paste_x2 - paste_x1
                region_height = paste_y2 - paste_y1
                extracted_region_resized = cv2.resize(extracted_region, (region_width, region_height))

                # Paste the resized extracted region into the frame
                frame[paste_y1:paste_y2, paste_x1:paste_x2] = extracted_region_resized
