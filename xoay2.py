...
# Đặt seed mặc định là 666
random.seed(666)
np.random.seed(666)
torch.manual_seed(666)

# --- Hàm xoay ảnh với tính toán kích thước bound (giữ nguyên toàn bộ nội dung) và tạo mặt nạ ---
def rotate_image_bound_with_mask(image, angle, pivot='bottom_left'):
    """
    Xoay ảnh quanh trục quay được chỉ định (mặc định: góc dưới cùng bên trái)
    và trả về ảnh xoay với kích thước bound mới cùng với mặt nạ (mask) tương ứng.
    """
    (h, w) = image.shape[:2]
    ...
    # Tính tọa độ 4 đỉnh của ảnh gốc
    corners = np.array([
        [0, 0],
        [w, 0],
        [w, h],
    ...
    transformed_corners = M.dot(corners_hom.T).T

    # Tính kích thước ảnh xoay mới (bound)
    ...
    # Điều chỉnh ma trận xoay để bù dịch sao cho ảnh xoay nằm hoàn toàn trong khung mới
    M[0, 2] -= min_x
    M[1, 2] -= min_y

    rotated = cv2.warpAffine(image, M, (new_w, new_h), borderValue=(0, 0, 0))
    # Tạo mặt nạ: toàn bộ ảnh gốc có giá trị 255
...
def paste_object_with_rotation_effect(nth_person, tracking_data, inpainted_frames_folder, original_frames_folder, gendata_folder, max_rotation_angle=90):
    ...
    # Khởi tạo khoảng thời gian giữa các frame (dt) với FPS = 24
    dt = 1.0 / 24.0
    current_angle = 0.0             # Góc xoay ban đầu
    ...
    for data in tracking_data:
        ...
        scale_factor = 30.0  # Điều chỉnh để hiệu ứng xoay rõ ràng hơn
        ...
        # Xoay đối tượng với trục quay là góc dưới cùng bên trái
        rotated_box, rotated_mask = rotate_image_bound_with_mask(extracted_box, angle, pivot='bottom_left')
        ...
        # Tính vị trí dán sao cho tâm của rotated_box trùng với center của bbox ban đầu
...
def main():
    ...
    # Gọi hàm dán đối tượng với hiệu ứng xoay, sử dụng góc xoay được cập nhật dựa trên gia tốc góc
