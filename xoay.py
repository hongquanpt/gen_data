...
def paste_object_with_rotation_effect(nth_person, tracking_data, inpainted_frames_folder, original_frames_folder, gendata_folder, rotation_increment=5, max_rotation_angle=90):
    """
    Với mỗi khung hình theo thông tin tracking_data:
      ...
      - Dán đối tượng đã xoay lên khung hình nền (inpainted) sao cho tâm của đối tượng xoay
        trùng với tâm của bbox ban đầu. Trong đó chỉ dán các pixel có mặt nạ (không dán vùng padding).
      => Lưu ý: Khi góc xoay đạt đến 90 độ, dừng xử lý các khung hình tiếp theo.
    """
    ...
    for data in tracking_data:
        ...
        # Tính góc xoay tăng dần dựa theo số frame xử lý
        angle = min(processed_count * rotation_increment, max_rotation_angle)
        processed_count += 1
        ...
        if dest_x2 > dest_x1 and dest_y2 > dest_y1:
            # Lấy vùng của đối tượng và mặt nạ tương ứng
            ...
            # Lấy vùng nền hiện tại
            dest_roi = inpainted_frame[dest_y1:dest_y2, dest_x1:dest_x2]
            # Dán đối tượng: với những pixel có mask > 0, thay thế bằng pixel của đối tượng; còn lại giữ nguyên nền.
        ...
        # Nếu góc xoay đã đạt đến 90 độ, dừng xử lý các khung hình tiếp theo.
...
def main():
    ...
    # Gọi hàm dán đối tượng với hiệu ứng xoay, dán đối tượng không ghi đè lên nền bằng cách sử dụng mask
    paste_object_with_rotation_effect(
        ...
        rotation_increment=5,    # Mỗi khung hình xoay thêm 5 độ
